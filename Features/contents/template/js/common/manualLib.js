...
function TocObject()
{
  ...
  this.setPageNaviObject = function(url)
  {
    var id = this.getPageNaviObjectId(url);
    ...
    if(rootObj != null)
    {
      ...
      for(var i = 0; i < this.getDataCount(); i++)
      {
        ...
        else
        {
          ...
          else if(!this.viewFileIcon || !data.isViewIcon())
...
Reassemble.reassembleImpl = function()
{
  ...
  if(Reassemble.objectTocData != null && typeof jQuery === "function")
  {
    ...
    var currentId = manager.getPageNaviObjectId(location.href);
    var current = manager.getPageNaviObject(currentId);
    if(current == null)
    {
      currentId = manager.getPageNaviObjectId(location.pathname);
      current = manager.getPageNaviObject(currentId)
    }
    if(current != null)
    {
      ...
      {
        var linktext = "";
        var obj = manager.getPageNaviObject(current.prevId);
        if(obj != null)
        {
      ...
      {
        var linktext = "";
        var obj = manager.getPageNaviObject(current.nextId);
        if(obj != null)
        {
          if(!obj.disablePrintPage)
          {
  ...
  if(tempObj != null)
  {
    ...
    temp.bind("load",function()
    {
      ...
      $(Reassemble.viewerElement).width(winWidth).height(winHeight).offset({
        left: (clientWidth - winWidth) / 2,
        top: (clientHeight - winHeight) / 2
      });
      ...
      $(Reassemble.viewerElement).css({
        ...
        borderRadius: "8px",
        boxShadow: "rgba(0,0,0,0.5) 4px 4px"
      });
